...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
		...
		UUser hasUser = userService.getUserByLoninName(map);
        if (hasUser != null) {
            // 若存在，将此用户存放到登录认证info中，无需自己做密码对比，Shiro会为我们进行密码对比校验
            List<URole> rlist = roleService.getRolesByUserId(hasUser.getUserId());//获取用户角色
            List<UPermission> plist = roleService.listByUserId(hasUser.getUserId());//获取用户权限
            ...
            for (UPermission uPermission : plist) {
            	perminsStrlist.add(uPermission.getMenuName());
			}

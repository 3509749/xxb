...
(function ($) {
	$.fn.json2form = function(config) { 
		...
		if(config.data){//input text password hidden button reset submit checkbox radio select textarea
			$("#"+config.elem+" input,select,textarea,label").each(function(){
				var elemType=$(this).attr("type")==undefined?this.type:$(this).attr("type");
				if(elemType == 'button')return true;//在jquery中，相当于continue
				var elemName=$(this).attr("name");
				var elemId=$(this).attr("id");
				if((!elemId || elemId=='undefined')&&(!elemName || elemName=='undefined'))return true;//在jquery中，相当于continue
				//alert(elemName);
				//var elemData=config.data[elemName];     //delete by wanghua
				var elemArrStr = elemName||elemId;
				var elemNameArr = elemArrStr.split('.');	  //add by wanghua		
				var elemData=config.data[elemNameArr[0]]; //add by wanghua
				if(typeof(elemData) == "undefined") return true;
				...
				//if(elemData) //delete by wanghua
				{
					switch(elemType){
						...
						case "select-one":{
							...
							$("#"+elemId).combobox('select', elemData);
							break;
						}
						case "label":{
							$(this).html(elemData)
						...
						case "label-time":{
							$(this).html(new Date(elemData).toLocaleString())
							break;
						}
						case "text":
						case "password":
						...
						case "textarea":
						case "submit":{
							if(elemArrStr.indexOf("_easyui_textbox_input") != -1) break;
							if(!elemData) elemData = "";

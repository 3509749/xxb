(function($){
	$.fn.slider = function (options) {
		var $this = $(this);
		var _this = this;
		var width_all=document.body.clientWidth;
		return this.each (function(){
			var defaultValue = {
				width: 					width_all, 					// 幻灯片的宽度
				height:     			665, 					// 幻灯片的高度
			...
			defaultValue.init = function() {
				...
				if(opt.showOrigin){
					...
					if(opt.originPosition === 'right') _this.slider_origin = $('<span class="slider-origin"></span>').css({right:'45%',transform:'translate(0,0)','-webkit-transform':'translate(0,0)','-moz-transform':'translate(0,0)'}).appendTo(_this.cpt_slider);
					for(var i = 0;i< opt.source.length;i++){
